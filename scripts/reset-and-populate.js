...
async function resetAndPopulate() {
  ...
  try {
    ...
    // Step 2: Add treatments (services)
    ...
    // Step 3: Add products (inventory items)
    ...
    // Step 4: Add realistic patients
    console.log('👥 Adding realistic patients...')
    const patients = [
      {
        first_name: 'Rajesh',
        last_name: 'Kumar',
    ...
    // Step 5: Create comprehensive visit scenarios with realistic workflow
    console.log('📋 Creating realistic visit scenarios...')

    const visitScenarios = [
      // Patient 1: Rajesh Kumar - Hypertension management (multiple visits over time)
      [
        // Visit 1: Initial consultation (3 months ago)
        {
        ...
        // Visit 2: Follow-up (2 months ago)
        {
          ...
          notes: 'Patient tolerating new medication well. Continue current regimen.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 },
        ...
        // Visit 3: Recent follow-up (2 weeks ago)
        {
          ...
          notes: 'Continue current treatment plan. Next follow-up in 3 months.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 },
            { treatment_name: 'Blood Pressure Check', quantity: 1, unit_price_cents: 20000 }
      ...
      // Patient 2: Anita Sharma - Diabetes management
      [
        // Visit 1: Initial diagnosis (2 months ago)
        {
          visit_date: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          visit_type: 'consultation',
 ...
 'Patient motivated for lifestyle changes. Good candidate for diabetes education program.',
          status: 'completed',
          treatments: [
        ...
        // Visit 2: Recent follow-up (1 week ago)
        {
 ...
 'Consider adding DPP-4 inhibitor if HbA1c doesn\'t improve further.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 },
            { treatment_name: 'Blood Test (CBC)', quantity: 1, unit_price_cents: 80000 }
      ...
      // Patient 3: Arjun Patel - Cardiac care
      [
        // Visit 1: Post-surgery follow-up (1 month ago)
        {
 ...
 'Surgical wounds healing well. Cardiac rehabilitation program started.',
          status: 'completed',
          treatments: [
        ...
        // Visit 2: Recent cardiology consultation (3 days ago)
        {
 ...
 'Patient stable. ECHO scheduled next week. Rate control achieved.',
          status: 'completed',
          treatments: [
            { treatment_name: 'ECG', quantity: 1, unit_price_cents: 100000 },
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 }
      ...
      // Patient 4: Priya Singh - Thyroid issues
      [
        // Visit 1: Initial consultation (1 month ago)
        {
          visit_date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          visit_type: 'consultation',
 ...
 'Patient advised on medication timing and follow-up in 6 weeks.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 },
            { treatment_name: 'Blood Test (CBC)', quantity: 1, unit_price_cents: 80000 }
      ...
      // Patient 5: Vikram Reddy - Asthma management
      [
        // Visit 1: Emergency visit (1 week ago)
        {
          visit_date: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          ...
          notes: 'Patient stabilized. Discharged with oral prednisone taper and increased inhaler dose.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 },
 ...
 'Continue current inhaler regimen. Return if symptoms worsen.',
          status: 'completed',
          treatments: [
            { treatment_name: 'General Consultation', quantity: 1, unit_price_cents: 50000 }
          ]
        ...
        try {
          ...
          const { data: visit, error: visitError } = await supabase
            ...
            .insert({
              ...
              treatment_notes: visitData.treatment_notes:notes,
          ...
          if (visitData.treatments && visitData.treatments.length > 0) {
            for (const treatment of visitData.treatments) {
              const treatmentData = t.name === treatment.treatment_notes:name)
              if (!treatmentData) {
                console.error(`❌ Treatment not found: ${treatment.treatment_notes:name}`)
                continue
              }
          ...
          if (visitData.treatments && visitData.treatments.length > 0) {
            ...
            if (billItems.length > 0) {
              ...
              if (billNumberError) {
                console.error(`❌ Error generating bill number:`, billNumberError.message)
                continue
              }
              ...
              if (billError) {
                ...
                const billItemsData = billItems.map(item => ({
                  bill_id: bill.id,
                  treatment_name: item.treatment_notes:name,
          ...
          // Handle inventory purchases (if any)
          if (visitData.inventory_purchases && visitData.inventory_purchases.length > 0) {
            for (const purchase of visitData.inventory_purchases) {
              const productData = 'p.name' === purchase.product_name)
              if (!productData) {
    ...
    // Step 6: Add realistic expenses
    console.log('\n💸 Adding realistic clinic expenses...')
